...
import { QualityProcessRecord, FileServiceProxy, QualityServiceProxy, QualityKPICheck } from '../../../shared/service-proxies-hdApp/service-proxies-hdApp';
...
/**
 * Generated class for the SupervisorCheckDetailPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-supervisor-check-detail',
  templateUrl: 'supervisor-check-detail.html',
})
export class SupervisorCheckDetailPage {
  ...
  getData() {
    this.qualityServiceProxy.getQualityById(this.id).subscribe(data => {
  ...
  saveRecord() {
    ...
    value.isOk = this.newRecord.isOk;
  ...
  checkInfo() {
    this.navCtrl.push('AppointPage', { id: this.id, type: 2 });
  }

  /**
 *添加整改情况
 * @memberof EngineerQualityDetailPage
 */
  addCheck() {
  ...
  sendEmp() {
    this.navCtrl.push('AppointConstructionPage',{id: this.id, type: '2'});
  }
}
