...
import { QualityProcessRecord, FileServiceProxy, QualityServiceProxy, QualityDayCheck } from '../../shared/service-proxies-hdApp/service-proxies-hdApp';
...
/**
 * Generated class for the QualityPatrolDetailPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-quality-patrol-detail',
  templateUrl: 'quality-patrol-detail.html',
})
export class QualityPatrolDetailPage {
  ...
  getData() {
    this.qualityServiceProxy.getQualityDayChecksById(this.id).subscribe(data => {
  ...
  showInput() {
    let popover = this.popoverCtrl.create("QualityCheckInputPage", {
      isOk: this.newRecord.isOk,
      detail: this.newRecord.supervisorDetail,
    ...
    popover.onDidDismiss(data => {
      if (data) {
        if (data.isSubmit) {
          this.newRecord.isOk = data.isOk;
          this.newRecord.supervisorDetail = data.detail;
          this.saveRecord();
        }
      }
    })
