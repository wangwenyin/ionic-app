...
import { QualityAppentFileInfo, EmployeeServiceProxy, FileServiceProxy, QualityServiceProxy, Employee, QualityNoKPICheck } from '../../../shared/service-proxies-hdApp/service-proxies-hdApp';
...
/**
 * Generated class for the DailyQualityPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-daily-quality',
  templateUrl: 'daily-quality.html',
})
export class DailyQualityPage {
  ...
  onSaveRecord() {
    if (this.changeTable && this.type && this.input.subentryId && this.input.checkValue && this.input.constructionUnit
      && this.userItem && this.input.detail) {
  ...
  saveRecord(imgList) {
    this.input.state = '1';
    this.qualityServiceProxy.createQualityNoKPICheck(this.input).subscribe(data => {
  ...
  changeChargeStatus(e) {
    if (e && this.input.isBroken) {
      this.input.isBroken = false;
    }
    if (!e && !this.input.isBroken) {
      this.input.isBroken = true;
    }
  }

  changeBrokeStatus(e) {
    if (e && this.input.isRectification) {
      this.input.isRectification = false;
    }
    if (!e && !this.input.isRectification) {
      this.input.isRectification = true;
    }
  }
  /**
   *楼层改变
